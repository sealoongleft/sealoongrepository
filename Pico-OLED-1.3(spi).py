...
class OLED_1inch3(framebuf.FrameBuffer):
    ...
    def init_display(self):
     ...
     #  self.write_cmd(0x10)   #set higher column address 没什么用 

        self.write_cmd(0xB0)   #set page address       左右滚动
      
        self.write_cmd(0xdc)    #et display start line 垂直滚动
        self.write_cmd(0x00) 
        self.write_cmd(0x81)    #contract control 
        self.write_cmd(0x00)    #128   亮度调节
        self.write_cmd(0x21)    # Set Memory addressing mode (0x20/0x21) 0x21isvertical address mode
        if self.rotate == 0:
            self.write_cmd(0xa0)    #set segment remap 从上到下
        elif self.rotate == 180:
            self.write_cmd(0xa1)    #从下到上
        self.write_cmd(0xc0)    #Com scan direction c0(com0__com123),a0
        self.write_cmd(0xaa)   #Disable Entire Display On (0xA4/0xA5) 

        self.write_cmd(0xa6)    #normal / reverse   a7
        self.write_cmd(0xa8)    #multiplex ratio
        self.write_cmd(0x40)    #com0到64被使能
     #  self.write_cmd(0x3f)    #duty = 1/64
        ...
        self.write_cmd(0x81)    #Set DC-DC enable (a=0:disable; a=1:enable)
        self.write_cmd(0XAF)
    def show(self):
       # self.write_cmd(0xb0)   #first page
...
if __name__=='__main__':
    ...
    while(1):
        ...
        if(keyB.value() == 0):
            ...
            if OLED.rotate == 0:
                OLED.fill_rect(0,43,128,20,OLED.balck)
            else:
                OLED.fill_rect(0,0,128,20,OLED.balck)
